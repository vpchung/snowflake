...
def query_kpis() -> str:
    """Folder and files counts in the MC2 Center project."""
    return f"""
        WITH
            {CTE_NON_SAGERS},
            project_files AS (
            ...
            project_file_handles AS (
                SELECT DISTINCT file_handle_id
                FROM project_files
                WHERE name NOT ILIKE '{MANIFEST_FILTER}'
            ),
            events AS (
                SELECT
                    dl.user_id,
                ...
                INNER JOIN
                    project_file_handles ON dl.file_handle_id = project_file_handles.file_handle_id
                WHERE
        ...
        SELECT
            ...
            AND name NOT ILIKE '{MANIFEST_FILTER}') AS total_folders,
...
def query_monthly_downloads() -> str:
    """Monthly external download counts for the MC2 Center project."""
    return f"""
        WITH
            {CTE_NON_SAGERS},
            project_files AS (
...
def query_new_external_users() -> str:
    """Monthly count of users whose first download fell in each period."""
    return f"""
        WITH
            {CTE_NON_SAGERS},
            project_files AS (
        ...
        WITH
            ...
            project_files AS (
                ...
                WHERE
                    ...
                    AND child.name NOT ILIKE '{MANIFEST_FILTER}'
            )
...
def _cell_kpis():
    ...
    try:
        ...
        col4.metric(
            "External Users",
            f"{int(row['TOTAL_EXTERNAL_USERS']):,}",
        )
        ...
        download1.metric(
            ...
            f"{int(row['TOTAL_EXTERNAL_DOWNLOADS']):,}",
        )
        download2.metric(
    ...
    with st.container(border=True):
        with st.container(
            ...
            with st.container(height=80, border=False, vertical_alignment="center"):
                st.markdown("### Monthly External User Downloads")
            if st.button(
                ":material/refresh:",
                type="tertiary",
...
def _cell_file_type_breakdown():
    with st.container(border=True):
        ...
        try:
            ...
            df = rename_duplicate_columns(df)
            st.caption(f"{int(df['EXTERNAL_DOWNLOADS'].sum()):,} external user downloads in view")
            st.dataframe(
                df,
                width="stretch",
                hide_index=True,
    ...
    with st.container(border=True):
        with st.container(
            ...
            if st.button(
                ...
                if "mc2_files_browser_qid" in st.session_state:
                    del st.session_state["mc2_files_browser_qid"]

        # Reuses the cached result if available, otherwise executes the query and caches the result
        current_qid = execute_query(query_files_browser())
        if (
            "mc2_files_browser_df" not in st.session_state
            or st.session_state.get("mc2_files_browser_qid") != current_qid
        ):
            try:
                ...
                st.session_state["mc2_files_browser_qid"] = current_qid
            except Exception as e:
                st.error(f"Error: {e}")
                return
        ...
        if selected_folders:
            mask &= df["FOLDER_NAME"].isin(selected_folders)
...
def render():
    st.info(
        """
        This tab displays metrics specific to the **Multi-Consortia Coordinating (MC2) Center**
        ([syn7080714](https://www.synapse.org/Synapse:syn7080714)).
        """,
        icon=":material/info:",
    )
    _cell_kpis()
